...
def plug_all(adapters: list) -> list:
    ...
    and collects the joltage difference for each adapter in a list of lists, the index of the outer list is
    the joltage difference, the values in the inner list are the joltage rating of an adapter.
    """
    current = 0
    diffs = []
    for i in range(4):
    ...
    for entry in adapters:
        diff = entry - current
        if 0 <= diff <= 3:
            diffs[diff].append(entry)
            current = entry
        else:
            return []
    diffs[3].append(current+3)
    return diffs


class Day10(AbstractDay.AbstractDay):
    """
    ...
    would need to have a joltage rating of 1, 2, or 3 jolts. Of these, only one you have is an adapter rated
    ...
    ways they can be arranged. Every arrangement needs to connect the charging outlet to your device. The previous
    ...
    def part1(self) -> str:
        if len(self.entries) == 0:
            return 'No result'
        diffs = plug_all(self.entries)
        return str(len(diffs[1])*len(diffs[3]))
    ...
    def part2(self) -> str:
        ...
        for entry in self.entries:
            c = sum(possibilities[entry - x] for x in (1, 2, 3))
            possibilities[entry] = c
        return str(possibilities[self.entries[-1]])
