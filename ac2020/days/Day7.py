...
class Day7(AbstractDay.AbstractDay):
    """
    ...
    **How many individual bags are required inside your single `shiny gold` bag?**
    ...
    def _set_input(self, input_: str):
        """
        Splitting by lines. Each line contains a rule, the line is split by ' contain ' to get the left hand side and
        right hand side. Every 'bag' and 'bags' are thrown out, and every 's' is stripped from the right from the
        colors. I'm guessing there are no colors ending with the letter 's' :)
        """
        self.rules = {}
        if len(input_) != 0:
            rows = input_.split('\n')
            for entry in rows:
                rule = entry.replace('.', '').split(' contain ')
                statements = rule[1].split(', ')
                key = rule[0].replace(' bags', '').rstrip('s')
                self.rules[key] = []
                for statement in statements:
                    self.rules[key].append(statement.replace(' bag', '').strip().rstrip('s'))

    def part1(self) -> str:
        options = set()
        for rule in self.rules:
            if any('shiny gold' in s for s in self.rules[rule]):
                options.add(rule)
        expanded = True
        while expanded:
            ...
            for option in current_options:
                for rule in self.rules:
                    if any(option in s for s in self.rules[rule]):
                        options.add(rule)
            expanded = len(current_options) != len(options)
    ...
    def part2(self) -> str:
        ...
        """
        if 'shiny gold' not in self.rules:
            return 'No result'
        return str(self.whats_in_the_bag(self.rules, 'shiny gold'))

    def whats_in_the_bag(self, rules, color) -> int:
        summa = 0
        for rule in rules[color]:
            split = rule.split(' ', 1)
            if split[0] != 'no':
                multiplier = int(split[0])
                inside = self.whats_in_the_bag(rules, split[1])
                if inside == -1:
                    summa += multiplier
                if inside != -1:
                    summa += multiplier * inside
                    summa += multiplier
            else:
                return -1
        return summa
