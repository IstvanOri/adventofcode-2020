...
class Day13(AbstractDay.AbstractDay):
    """
    Advent of Code 2020 - Day 13
    ============================
    Your ferry can make it safely to a nearby port, but it won't get much further. When you call to book another
    ship, you discover that no ships embark from that port to your vacation island. You'll need to get from the port
    to the nearest airport.

    Fortunately, a shuttle bus service is available to bring you from the sea port to the airport! Each bus has
    ...
    each bus travels to the airport, then various other locations, and finally returns to the sea port to
    ...
    port at timestamps 0, 5, 10, 15, and so on. The bus with ID 11 departs at 0, 11, 22, 33, and so on. If you
    ...
    timestamp you could depart on a bus. The second line lists the bus IDs that are in service according to the
    shuttle company; entries that show x must be out of service, so you decide to ignore them.
    ...
    need to wait 944 - 939 = 5 minutes before it departs. Multiplying the bus ID by the number of minutes you'd
    ...
    def _set_input(self, input_: str):
        ...
        self.timestamp = 0
        if len(input_) != 0:
            self.buses = list(map(int, [-1 if x == 'x' else x for x in input_.split('\n')[1].split(',')]))
            self.timestamp = int(input_.split('\n')[0])
    ...
    def part1(self) -> str:
        ...
        first = -1
        bus_id = 0
        for i in range(self.timestamp, self.timestamp + max(self.buses)):
            schedule[i] = []
            for bus in self.buses:
                if bus > -1 and i % bus == 0:
                    if first < 0:
                        first = i
                        bus_id = bus
                    schedule[i].append(bus)
        return str((first - self.timestamp) * bus_id)

    def part2(self) -> str:
        """
        Search by sieving variant of Chinese remainder theory
        ...
        for i, bus in enumerate(self.buses):
            if bus > 0:
                while (bingo + i) % bus:
                    bingo += abs(math.prod(self.buses[:i]))
        return str(bingo)
