...
from ac2020.days.antlr.Day19Part1ExampleLexer import Day19Part1ExampleLexer
from ac2020.days.antlr.Day19Part1ExampleListener import Day19Part1ExampleListener
from ac2020.days.antlr.Day19Part1ExampleParser import Day19Part1ExampleParser
from ac2020.days.antlr.Day19Part2ExampleLexer import Day19Part2ExampleLexer
from ac2020.days.antlr.Day19Part2ExampleListener import Day19Part2ExampleListener
from ac2020.days.antlr.Day19Part2ExampleParser import Day19Part2ExampleParser
from ac2020.io import InputReader


class FailOnErrorListener(ErrorListener):

    def __init__(self):
        super(FailOnErrorListener, self).__init__()
...
class Day19(AbstractDay.AbstractDay):
    """
    Advent of Code 2020 - Day 19
    ============================

    You land in an airport surrounded by dense forest. As you walk to your high-speed train,
    the Elves at the Mythical Information Bureau contact you again. They think their satellite has collected
    ...
    one list of sub-rules must match. (The ones that match might be different each time the rule is encountered.)
    For example, the rule 2: 1 3 | 3 1 means that to match rule 2, the text being checked must match
    ...
    matches will be finite. Since rule 1 matches a and rule 3 matches b, rule 2 matches either ab or ba.
    ...
    so you can determine which are valid and which are corrupted. Including the rules and the messages together,
    ...
    ababbb and abbbab match, but bababa, aaabbb, and aaaabbb do not, producing the answer 2.
    ...
    def _set_input(self, input_: str):
        self.entries = InputReader.read_line_by_line(input_)

    def part1(self) -> str:
        """The solution is powered by ANTLR (installed python3 runtime is required. Use pip.)"""
        goods = 0
        for entry in self.entries:
            try:
                input_stream = InputStream(entry)
                lexer = Day19Part1ExampleLexer(input_stream)
                lexer.addErrorListener(FailOnErrorListener)
                stream = CommonTokenStream(lexer)
                parser = Day19Part1ExampleParser(stream)
                parser.addErrorListener(FailOnErrorListener)
                tree = parser.r0()
                walker = ParseTreeWalker()
                walker.walk(Day19Part1ExampleListener(), tree)
                goods += 1
            except Exception:
                pass
        return str(goods)
    ...
    def part2(self) -> str:
        ...
        for entry in self.entries:
            try:
                input_stream = InputStream(entry)
                lexer = Day19Part2ExampleLexer(input_stream)
                lexer.addErrorListener(FailOnErrorListener)
                stream = CommonTokenStream(lexer)
                parser = Day19Part2ExampleParser(stream)
                parser.addErrorListener(FailOnErrorListener)
                tree = parser.r0()
                walker = ParseTreeWalker()
                walker.walk(Day19Part2ExampleListener(), tree)
                goods += 1
            except Exception:
                pass
        return str(goods)
