...
class Day9(AbstractDay.AbstractDay):
    ...
    any two of the 25 immediately previous numbers. The two numbers will have different values, and there might
    be more than one such pair.

    For example, suppose your preamble consists of the numbers 1 through 25 in a random order. To be valid, the
    next number must be the sum of two of those numbers:
    ...
    was 20. Now, for the next number to be valid, there needs to be some pair of numbers among 1-19, 21-25, or 45
    that add up to it:
    ...
    (after the preamble) which is not the sum of two of the 25 numbers before it. What is the first number
    ...
    def part1(self) -> str:
        for i in range(self._PREAMBLE_, len(self.entries)):
            all_combinations = combinations(self.entries[i-self._PREAMBLE_:i], 2)
            found = False
            for combination in all_combinations:
                if combination[0] + combination[1] == self.entries[i]:
                    found = True
            if not found:
                self.part1_result = self.entries[i]
                return str(self.entries[i])
        return 'No result'

    def part2(self) -> str:
        for i in range(2, len(self.entries)):
            for j in range(len(self.entries) - i):
                sublist = self.entries[j:j+i]
                if sum(sublist) == self.part1_result:
