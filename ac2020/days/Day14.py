...
class Day14(AbstractDay.AbstractDay):
    """
    Advent of Code 2020 - Day 14
    ============================

    As your ferry approaches the sea port, the captain asks for your help again. The computer system
    that runs this port isn't compatible with the docking program on the ferry, so the docking parameters aren't
    ...
    The current bitmask is applied to values immediately before they are written to memory: a 0 or 1
    ...
    bits in every written value: the 2s bit is overwritten with 0, and the 64s bit is overwritten with 1.
    ...
    every address.) In the above example, only two values in memory are not zero - 101 (at address 7)
    ...
    memory address decoder. Immediately before a value is written to memory, each bit in the bitmask modifies
    ...
    this means the floating bits will take on all possible values, potentially causing many memory addresses
    ...
    are floating. Floating bits take on every possible combination of values; with two floating bits, four
    ...
    def _set_input(self, input_: str):
        ...
        self.values = []
        if len(input_) != 0:
            self.commands = list(x.split(' = ')[0] for x in input_.split('\n'))
            self.values = list(x.split(' = ')[1] for x in input_.split('\n'))
    ...
    def part1(self) -> str:
        ...
        for i, command in enumerate(self.commands):
            ...
            if command.startswith('mem'):
                address = command.replace('mem[', '').replace(']', '')
                value = self.values[i]
                to_mask = list('{0:036b}'.format(int(value)))
                for j, c in enumerate(list(mask)):
                    if c != 'X':
                        to_mask[j] = c
                memory[address] = int(''.join(to_mask), 2)
        return str(sum(memory.values()))

    def part2(self) -> str:
        """
        for each memory access, the mask is loaded into an array (addresses) and the first occurrence of letter 'X'
        is changed to 0 and 1 and both value is added into that same array, until the state of the array
        relaxes (no change in length). After that the array is filtered for residual addresses (the ones that still
        contain 'X' are filtered out), then the remaining addresses are set to the given value in the memory.
        After all commands are performed, the content of the memory is summarized.

        :return: The sum of the content of the memory.
        """
        memory = defaultdict(int)
        mask = ''
        for i, command in enumerate(self.commands):
            ...
            if command.startswith('mem'):
                ...
                value = self.values[i]
                addresses = []
                to_mask = list('{0:036b}'.format(int(address)))
                for j, c in enumerate(list(mask)):
                    if c != '0':
                        to_mask[j] = c
                addresses.append(''.join(to_mask))
                prev_len = 0
                while prev_len < len(addresses):
                    prev_len = len(addresses)
                    for adr in addresses:
                        adr1 = adr.replace('X', '1', 1)
                        if adr1 not in addresses:
                            addresses.append(adr1)
                        adr0 = adr.replace('X', '0', 1)
                        if adr0 not in addresses:
                            addresses.append(adr0)
                addresses = list(filter(lambda x: 'X' not in x, addresses))
                for adr in addresses:
                    memory[int(''.join(adr), 2)] = int(value)
        return str(sum(memory.values()))
