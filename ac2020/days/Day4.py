...
def is_true(lst: {}) -> bool:
    result: bool = True
    for item in lst:
        result = result and lst[item]
    return result


class Day4(AbstractDay.AbstractDay):
    """
    ...
    North Pole Credentials, not a passport at all! Surely, nobody would mind if you made the system temporarily
    ...
    def part1(self) -> str:
        ...
        for entry in self.entries:
            chk = {'byr': False, 'iyr': False, 'eyr': False, 'hgt': False, 'hcl': False, 'ecl': False, 'pid': False, 'cid': True}
    ...
    def part2(self) -> str:
        ...
        for entry in self.entries:
            ...
            for field in entry.split():
                f = field.split(':')
                if f[0] == 'byr':
                    if 2002 >= int(f[1]) >= 1920:
                        chk['byr'] = True
                if f[0] == 'iyr':
                    if 2020 >= int(f[1]) >= 2010:
                        chk['iyr'] = True
                if f[0] == 'eyr':
                    if 2030 >= int(f[1]) >= 2020:
                        chk['eyr'] = True
                if f[0] == 'hgt':
                    if not f[1].isnumeric():
                        h = f[1].replace('cm', '')
                        if h.isnumeric():
                            if 193 >= int(h) >= 150:
                                chk['hgt'] = True
