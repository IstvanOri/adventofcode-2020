...
class Day15(AbstractDay.AbstractDay):
    """
    ...
    list of starting numbers (your puzzle input). Then, each turn consists of considering the most recently
    spoken number:
    ...
    (the previous turn, 4) and the turn number of the time it was most recently spoken before then (turn 1).
    ...
    def part1(self) -> str:
        if len(self.entries) == 0:
            return 'No result'
        numbers_spoken = list(self.entries)
        while len(numbers_spoken) < 2020:
            try:
                index = list(reversed(numbers_spoken[:-1])).index(numbers_spoken[-1])
                numbers_spoken.append(index + 1)
            except ValueError:
                numbers_spoken.append(0)
        return str(numbers_spoken[-1])
