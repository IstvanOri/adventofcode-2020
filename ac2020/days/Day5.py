...
def calc_id(entry):
    ...
    for i in entry:
        ...
        if i == 'R':
            c = c + math.ceil((d - c) / 2)
        if i == 'L':
            d = d - math.floor((d - c) / 2)
    seat_id = a * 8 + c
    return seat_id


class Day5(AbstractDay.AbstractDay):
    """
    ...
    list of rows; the first letter indicates whether the seat is in the front (0 through 63) or the back
    64 through 127). The next letter indicates which half of that region the seat is in, and so on until you're left
    ith exactly one row.
