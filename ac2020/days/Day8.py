...
class Day8(AbstractDay.AbstractDay):
    """
    ...
    next instruction to the other `acc +1` near the bottom. After it increases the accumulator from 1 to 2, `jmp -4`
    executes, setting the next instruction to the only `acc +3`. It sets the accumulator to 5, and `jmp -3` causes
    ...
    **What is the value of the accumulator after the program terminates?**
    ...
    def _set_input(self, input_: str):
        """
        splits the input by lines and splits each line to command and parameter.
        """
        if len(input_) != 0:
            rows = input_.split('\n')
            self.command = []
    ...
    def run(self):
        """
        Iterates over the commands by executing the commands.

        :return: 'Terminated: n', 'Infinite: n', or 'Failed'
            Where Terminated means the 'program' halted, Infinite means that infinite loop is detected and the
            execution sopped, and Failed means that some error occurred. The n is the actual value of the accumulator.
        """
        pointer = 0
        acc = 0
        visited = []
        while pointer not in visited:
            if pointer == len(self.command):
                return 'Terminated: '+str(acc)
            if pointer not in range(len(self.command)):
                return 'Failed'
            visited.append(pointer)
            key = self.command[pointer]
            if key == 'acc':
                acc += self.params[pointer]
                pointer += 1
            elif key == 'jmp':
                pointer += self.params[pointer]
            else:
                pointer += 1
        return 'Infinite: '+str(acc)
    ...
    def part2(self) -> str:
        for i in range(len(self.command)):
            reset_to = self.command[i]
            if self.command[i] == 'nop':
                self.command[i] = 'jmp'
            if self.command[i] == 'jmp':
                self.command[i] = 'nop'
            result = self.run()
            if result.startswith('Terminated'):
                return result
            self.command[i] = reset_to
        return 'No result'
